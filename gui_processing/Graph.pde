class Graph
{
  ...
  void GraphAxis(int x, int y, int w, int h) {  // The main declaration function
    xPos = x;
    yPos = y;
    Width = w;
    Height = h;
     ...
     void DrawAxis(){
        ...
        fill(0); color(0);stroke(0);strokeWeight(1);
        int t=60;
       ...
       // text(Title,xPos+Width/5,yPos+20);                            // Heading Title
        ...
        text("Time (seconds)  -  ",xPos+Width/2,yPos+Height+t/1.5);                     // x-axis Label 
        
        rotate(-PI/2);                                               // rotate -90 degrees
        text("Distance (mm)",-yPos-Height/2,xPos-t*1.6+40);                   // y-axis Label  
  ...
  void LineGraph(float[] x, float[] y) {
    ...
    stroke(0);
    fill(0);
    rect(xPos+((t)-x[0])/(x[x.length-1]-x[0])*Width, 
      0,25,height);
  }
}